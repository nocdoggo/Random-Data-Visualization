...
train_1 = train_1[train_1.iloc[:,2] != 'M']
train_1 = train_1[train_1.iloc[:,2] != 'm']
train_1['Date'] = pd.to_datetime(train_1['Date'], errors='coerce')
type(train_1.iloc[0,0])
train_1.columns = ['Date', 'Time', 'temp']
train_1['temp'] = pd.to_numeric(train_1['temp'])
...
#plt.boxplot(train_1['Date'],train_1['temp'])
...
x1 = pd.to_datetime( '1989-04-20' )
x2 = pd.to_datetime( '2015-09-30' )

#pos = ( train_1['Date'] - x1 ).days

train_1['Date'] = (train_1['Date'] - x1).dt.days
#pos = pos.dt.days
#type(train_1.loc[i])
...
sns.boxplot(x="Date", y="temp", data=train_1,palette='rainbow')
...
#matplotlib.dates.date2num(train_1.iloc[2,0])
